 ...
 *
 * TODO: document what this really does!
 *
 **************************************************************************/
void get_pv_lookup_new(MATRIX_T* scores, int query_len, int targets_len, int range,
  ...
  for (start = query_len - 1; start >= 0; start--) {
    ...
    for (i = 0; (start + i) < query_len; i++) {
      ...
      if (pmf_matrix) set_matrix_row(address_index, pdf_new, pmf_matrix);
  ...
  for (index_query = 0; index_query < get_motif_length(query_motif); index_query++) {
    for (index_target = 0; index_target < get_motif_length(target_motif); index_target++) {
      ...
      for (index = 0; index < get_motif_alph_size(query_motif); index++) {
        ...
        RND(SCORE_NUMERATOR, RNDDIG, SCORE_NUMERATOR);
        //printf("SCORE_NUMERATOR = %.20f\n", SCORE_NUMERATOR);
        ...
        RND(SCORE_SQ_DENOMINATOR_1, RNDDIG, SCORE_SQ_DENOMINATOR_1);
        RND(SCORE_SQ_DENOMINATOR_2, RNDDIG, SCORE_SQ_DENOMINATOR_2);
        //printf(
        //  "SCORE_SQ_DENOMINATOR_1 = %.20f\n", 
        //  SCORE_SQ_DENOMINATOR_1
        //);
        //printf(
        //  "SCORE_SQ_DENOMINATOR_2 = %.20f\n", 
        //  SCORE_SQ_DENOMINATOR_2
        //);
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < query_count; i++, qdb_remain--) {
    ...
    pmf_matrix = opts.complete_scores ? allocate_matrix(num_pv_lookup_array, num_array_lookups) : NULL;   // UK
    if (pmf_matrix) init_matrix(0, pmf_matrix);
    // calculate pvalues
