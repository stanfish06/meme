...
void set_kmer(char** result, char* sequence, int start_idx, int k) {
  ...
  for (i = 0; i < k; ++i) {
    ...
    if (curr_idx < 0 || curr_idx > (int) strlen(sequence) - 1) {
      (*result)[i] = 'X';
    } else {
 ...
 * If yes, then will contain the mod name. Otherwise, it is set to NULL.
 */
void get_tsv_mod_array(ARRAYLST_T* mod_array, char** modless, char* sequence, const char *alph_letters, char* unprocessed_sequence) {
  ...
  *modless[0] = '\0';
  ...
  // Check that the mod weight is not empty due to invalid format.
  if (ptr2 == ptr1+1) {
    fprintf(stderr, "The sequence is in an invalid PSM format: '%s'.\n", unprocessed_sequence);
    exit(EXIT_FAILURE);
  }
  ...
  while (ptr1 && ptr2) {
    ...
    if (ptr1 > sequence_mod_iterator && strchr(alph_letters, *(ptr1-1))) {
      ...
      // make sure the mod contains a valid weight or label
      double dummy;
      int nval = sscanf(mod+1, "%lf", &dummy);
      if (nval != 1) {
        // see if mod is a valid label
        int mod_len = strlen(mod);
        char *new_mod = mm_malloc(mod_len + 1);
        new_mod[0] = mod[0];
        new_mod[1] = '\0';
        if (strcmp(mod+1, "Phospho (STY)") == 0) {
          strncat(new_mod, "Phospho-STY", mod_len);
        } else if (strcmp(mod+1, "Carbamidomethyl (C)") == 0) {
          strncat(new_mod, "Carbamidomethyl-C", mod_len);
        } else if (strcmp(mod+1, "Oxidation (M)") == 0) {
          strncat(new_mod, "Oxidation-M", mod_len);
        } else {
	  fprintf(stderr, "Illegal mod weight or label value '%s' in PSM '%s'.\n", mod+1, unprocessed_sequence);
	  exit(EXIT_FAILURE);
        }
        if (new_mod[1] != '\0') {
          free(mod);
      ...
      arraylst_put((int) strlen(*modless) - 1, mod, mod_array);
  ...
  strcat(*modless, sequence_mod_iterator);
...
void add_phospho_file_to_table(char* phospho_filename,
  ...
  while ((read = getline_nonempty(&line, &len, fp)) != -1) {
    ...
    if (unprocessed_sequence == NULL) {
      fprintf(stderr, "Skipping PTM line %d because it does not have at least %d fields.\n", 
        ptm_line_num, index_of_sequence+1);
      continue;
    }
    ...
    if (options->filetype != Psm) {
      ...
      int useq_len = strlen(unprocessed_sequence);
      if (useq_len > 1 && unprocessed_sequence[0] == '_') {
        // sequences are in _[Acetyl (Protein N-term)]AES[Phospho (STY)]PAFLSAK_ format (format name??)
        int seq_start = 0;
        // Remove starting underscore.
        seq_start = 1;
        // Remove N-terminal mod if present.
        if (strlen(unprocessed_sequence) > 2 && unprocessed_sequence[seq_start] == '[') {
          seq_start = strchr(unprocessed_sequence, ']') - unprocessed_sequence + 1;
        }
        int seq_len = useq_len - seq_start;
        // Remove ending underscore.
        if (unprocessed_sequence[useq_len-1] == '_') seq_len--;
        sequence = mm_malloc(seq_len + 1);
        strncpy(sequence, unprocessed_sequence + seq_start, seq_len);
        sequence[seq_len] = '\0';
      } else if (strlen(unprocessed_sequence) > 2 && unprocessed_sequence[1] == '.') {
        ...
        int pep_length = (int) strlen(unprocessed_sequence) - 4;
        strncat(sequence, unprocessed_sequence + 2, pep_length);
        if (unprocessed_sequence[(int) strlen(unprocessed_sequence) - 1] != '-') {
          strncat(sequence, unprocessed_sequence + (int) strlen(unprocessed_sequence) - 1, 1);
        }
      } else {
        int count = 0;
        int plusminus_idx = -1;
    ...
    if (options->filetype == Psm) {
      ...
      get_tsv_mod_array(mod_array, &modless, sequence, alph_letters, unprocessed_sequence);
      i_start = 0;
      i_end = seq_len;
  ...
  if (!found_mod && options->filetype == Psm) {
    fprintf(stderr, 
      ...
      "  \'%s\'.\n"
      "Check that the peptides are in an allowable PSM format\n"
  ...
  if (options->algorithm == Motifx || options->algorithm == Modl) {
    ...
    for (i = 0; i < arraylst_size(mod_table_keys); ++i) {
      ...
      for (j = 0; j < num_bg_sequences; ++j) {
        ...
        for (k = options->width/2; k < (int) strlen(protein) - options->width/2; ++k) {
