...
double pearson_correlation(
  ...
  int *permute_x		// permute the values in the x array using these indices
				// sets mse==null
)
{
  int i;
  ...
  if (permute_x) mse = NULL;

  int n_valid = 0;
  for (i=0; i<n; i++) {
    if (valid_list && !valid_list[i]) continue;
    x_i = permute_x ? x[permute_x[i]] : x[i];
    y_i = y[i];
    sx += x_i;
    sy += y_i;
    sxy += x_i*y_i;
  ...
  // Correct for roundoff and ensure that -1 <= r <= +1.
  RND(r, 5, r);
  if (r > 1) r = 1;
  if (r < -1) r = -1;
  ...
  if (log_pv) {
    ...
    if (n_valid < 3) {
      // Can't estimate on fewer than 3 points.
      *log_pv = 0;
    } else if (r < 1) {
      z = 0.5*sqrt(n_valid-3)*log((1+r)/(1-r));
      if (two_tailed) {
        // Two-tails of normal.
	*log_pv = log_erfc(fabs(z)/sqrt(2));
      } else {
        // Right tail of normal.
	*log_pv = log(0.5) + log_erfc(z/sqrt(2));	
      }
    } else {
