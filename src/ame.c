...
#define MIN_SEQS 3		// Require at least this many sequences in primary and control sets to prevent crashing.  Change min_seqs to match in website/js/ame.js if this changes.
...
void ame_getopt(int argc, char *argv[]) {
  const int num_options = 29; // change this if the number of options changes
  ...
  if (! options.text_only) {
    ...
    options.tsv_output = fopen(path, "w"); //TODO: check for errors: MEME doesn't either and we at least know we have a good directory
    myfree(path);
    if (! options.noseq) {
      path = make_path_to_file(options.outputdir, SEQ_FILENAME);
      options.seq_output = fopen(path, "w"); //TODO check for errors: MEME doesn't either and we at least know we have a good directory
    } 
    myfree(path);
  }
...
AME_RESULT_T *ame_do_pearson_test(
  ...
  for (i=min; i <= max; i++) {
    ...
    if (options.linreg_switchxy) {
      if (use_ranks) {
        rho = pearson_correlation(i, y, x, NULL, NULL, NULL, &log_pv, false, NULL, NULL);
      } else {
        rho = pearson_correlation(i, y, x, &m, &b, &mse, &log_pv, false, NULL, NULL);
      }
    } else {
      if (use_ranks) {
	rho = pearson_correlation(i, x, y, NULL, NULL, NULL, &log_pv, false, NULL, NULL);
      } else {
	rho = pearson_correlation(i, x, y, &m, &b, &mse, &log_pv, false, NULL, NULL);
      }
    }
...
AME_RESULT_T *ame_get_motif_score(
  ...
  for (i=0; i<num_seqs; i++) {
    ...
    // Round scores to 10 significant digits so they will be consistent across platforms
    // (especially cross-compiled Docker).
    RND(rankings[i]->pwm_score, 10, rankings[i]->pwm_score); 
