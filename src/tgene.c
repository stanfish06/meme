...
// Number or random derangements for estimating null distribution
#define N_PERMS 44		// number of derangements of N=5 tissues 
...
static void finish_link_table(
  ...
  if (n_histones == 0) {
    ...
    for (i_hist=0; i_hist<n_histones; i_hist++) {
      scores = resize_init_array(scores, n_links, 0);
      null_scores = resize_init_array(null_scores, n_links*n_perms, 0);
      int n_scores = 0;
      int n_null = 0;
...
static void get_correlations(
  ...
  // Set up for permuting expression values across tissues for null distribution.
  int **perms;
  if (n_perms > N_PERMS) {
    DEBUG_FMT(NORMAL_VERBOSE, "  Creating %d random derangements of expression across %d tissues.\n", n_perms, n_tissues);
    perms = (int **) mm_malloc(sizeof(int *) * n_perms);
    srand_mt(options->seed);					// Make sure the derangements are not affected by other things.
    for (i=0; i<n_perms; i++) {
      perms[i] = (int *)mm_malloc(sizeof(int) * n_tissues);
      for (j=0; j<n_tissues; j++) perms[i][j] = j;		// Set permutation to identity.
      SHUFFLE(perms[i], n_tissues);				// Make a permutation of the indices.
      // Convert permutation to derangement: no index in its original position.
      for (j=0; j<n_tissues; j++) {
	if (perms[i][j] == j) {
	  int rand_num = rand_int(n_tissues - 1) + 1;
	  int new_index = j + rand_num;
	  if (new_index >= n_tissues) new_index = new_index - n_tissues;
	  perms[i][j] = perms[i][new_index];
	  perms[i][new_index] = j;
	}
      }
    }
  } else {
    DEBUG_FMT(NORMAL_VERBOSE, "  Creating all %d derangements of expression across %d tissues.\n", n_perms, n_tissues);
    perms = get_derangements(n_tissues, n_perms);  
  }
  ...
  for (link_i=0; link_i<n_links; link_i++) {
    ...
    double rho = pearson_correlation(n_tissues, transcript->expr, locus->histone_level,
      NULL, NULL, NULL, NULL, true, options->valid_tissues[i_hist], NULL);
    ...
    for (i=0; i<n_perms; i++) {
      rho = pearson_correlation(n_tissues, transcript->expr, locus->histone_level,
        NULL, NULL, NULL, NULL, true, options->valid_tissues[i_hist], perms[i]);
...
int main(int argc, char *argv[]) {
  ...
  link_table.n_perms = options.n_tissues > 5 ? N_PERMS+1 : get_num_derangements(options.n_tissues);
