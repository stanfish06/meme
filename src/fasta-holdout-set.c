...
int main(int argc, char *argv[])
{
  ...
  (void) initialize_st_alphabet(options.alph);
  ...
  BOOL no_reset = False;			// don't set reset the random number generator
  ARRAY_T *back = NULL;				// space for the MEME-style background
  Multiseq *holdout_multiseq = NULL;
  Multiseq *train_multiseq = read_pos_neg_seqs(&options, do_rc, True, True, no_trim, set_back, no_reset, back, &holdout_multiseq);
  free_array(back);
