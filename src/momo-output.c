...
void momo_print_motifs(
  ...
  for (i = 0; i < arraylst_size(mod_table_keys); ++i) {
    ...
    for (j = 0; j < arraylst_size(motifs); ++j) {
      ...
      char *motif_name = motifid2regexp(motifid, 'X', false, true);
      ...
      if (options.algorithm == Motifx || options.algorithm == Modl) {
        motif_regexp = motifid2regexp(motifid, '.', true, false);
	...
	for (k = 0; k < options.width; k++) {
      ...
      motif_png[0] = '\0';
      strcat(motif_png, motif_name);
      strcat(motif_png, ".png");
      ...
      logo_file[0] = '\0';
      strcat(logo_file, options.output_dirname);
      strcat(logo_file, "/");
