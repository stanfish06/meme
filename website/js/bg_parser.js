...
BgParser.prototype.process_blob = function (blob, offset, chunk_size) {
  ...
  if (offset === 0 && typeof this.handler.begin == "function")
    this.handler.begin(blob.name, blob.size);
...
BgHandler.prototype.summary = function () {
  ...
  if (this.max_file_name_length != null && this.file_name != null && this.file_name.length > this.max_file_name_length) {
    add(true, "The background file name is too long: " + this.file_name.length + " > " + this.max_file_name_length + ".");
  }
  // file size error 
  if (this.max_file_size != null && this.file_size > this.max_file_size) {
    add(true, "Large file", ["File is over " + Math.trunc(this.file_size /(1000000)) + "MB but must be no greater than " + this.max_file_size/(1000000) + "MB"])
  }
  ...
  if (this.mismatches.faults() > 0) {
    ...
    add(true, "Alphabet is inconsistent", [reason]);
  }
...
BgHandler.prototype.begin = function (file_name, file_size) {
  this.reset();
...
BgHandler.prototype.error = function (offset, line, column, code, message, data) {
  switch (code) {
    ...
    case BgErrorType.ENCODING:
      ...
      break;
    case BgErrorType.FILE_NAME_LENGTH:
