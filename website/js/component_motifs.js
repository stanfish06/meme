...
var MotifInput = function(container, options) {
  ...
  this.embed_indicator = (this.embed_surround != null ? this.embed_surround.querySelector("span.indicator") : null);
  this.embed_popup = (this.embed_surround != null ? this.embed_surround.querySelector("div.popup") : null);
  this.embed_hin = (this.embed_surround != null ? this.embed_surround.querySelector("input.data") : null);
  this.embed_descr = (this.embed_surround != null ? this.embed_surround.querySelector("input.name"): null);
  ...
  if (this.embed_hin) {
    (new MotifParser({
      ...
      "begin": function(embed_descr) { me._embed_begin(me.embed_descr.value); },
...
MotifInput.prototype._file_update = function () {
  ...
  this.file_parser = new MotifParser({
    ...
    "begin": function(file_size, file_name) { me._file_begin(file_size, file_name); },
    "end": function () { me._file_end(); },
...
MotifInput.prototype._embed_error = function (is_error, message, reasons) {
  this._add_error_entry(this.embed_popup, is_error, message, reasons);
  this.embed_error |= is_error;
  if (this.embed_error) {
    substitute_classes(this.embed_surround, ["good", "warning"], ["error"]);
  } else {
    substitute_classes(this.embed_surround, ["good"], ["warning"]);
  }
};
 ...
 ******************************************************************************/
MotifInput.prototype._embed_begin = function (embed_descr) {
  substitute_classes(this.embed_surround, ["warning", "error"], ["good"]);
  this.embed_indicator.style.width = "100%";
  this.embed_popup.innerHTML = "";
  this.embed_error = false;
  var source_name = embed_descr.split(" ")[4];	// "motif number N from file_name"
  if (typeof this.options["max_file_name_len"] === "number" && source_name.length > this.options["max_file_name_len"]) {
    this._embed_error(true, "The name of the submitting file is too long.  Submitting file name must be no longer than "
      + (this.options["max_file_name_len"]) + " characters but is " + source_name.length + " characters. ");
  }
};
...
MotifInput.prototype._embed_end = function () {
  this.embed_indicator.style.width = "100%";
  if (!(this.embed_error)) substitute_classes(this.embed_surround, ["good"], []);
...
MotifInput.prototype._embed_motif_meta = function (info) {
  "use strict";
  var motif_id = info.motif_id;
  if (motif_id != null) {
    // Check the length of the motif_id.
    var motif_index = info.motif_index;
    if (motif_index == null) motif_index = this.get_motif_count()+1;
    if (typeof this.options["max_name_len"] === "number" && motif_id.length > this.options["max_name_len"]) {
      this._embed_error(true, "Error in submitted motifs: The ID of submitted motif number " + motif_index
        + " is too long.  Motif ID must be no longer than " + this.options["max_name_len"] 
        + " characters but is " + motif_id.length + " characters.");
    }
  } else {
 ...
 * and checks that the file name is not too long.
 ******************************************************************************/
MotifInput.prototype._file_begin = function (file_size, file_name) {
  "use strict";
  substitute_classes(this.file_surround, ["warning", "error"], ["good"]);
  this.file_indicator.style.width = "0%";
  this.file_popup.innerHTML = "";
  this.file_error = false;
  if (typeof this.options["max_file_size"] === "number" && file_size > this.options["max_file_size"]) {
    this._file_error(true, "Motif file is too large.  File size must be no larger than "
      + (this.options["max_file_size"]/1000000) + "MB but is over "
      + (Math.trunc(file_size/1000000)) + "MB" + ". ");
  }
  if (typeof this.options["max_file_name_len"] === "number" && file_name.length > this.options["max_file_name_len"]) {
    this._file_error(true, "Motif file name is too long.  File name must be no longer than "
      + this.options["max_file_name_len"] + " characters but is " + file_name.length + " characters. ");
  }
 ...
 * If info contains motif_id, just checks it for valid length and skips
 * all other steps.
 ******************************************************************************/
MotifInput.prototype._file_motif_meta = function (info) {
  ...
  if (motif_id != null) {
    ...
    if (motif_index == null) motif_index = this.get_motif_count()+1;
    if (typeof this.options["max_name_len"] === "number" && motif_id.length > this.options["max_name_len"]) {
      this._file_error(true, "The ID of motif number " + motif_index
        + " is too long.  Motif ID must be no longer than " + this.options["max_name_len"] 
        + " characters but is " + motif_id.length + " characters.");
    }
  } else {
