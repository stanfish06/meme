...
function check() {
  "use strict";
  var alphs = null;
  var min_seqs = 3;		// Change MIN_SEQS in src/ame.c too if this changes.
  var num_sequences = 0;
  ...
  if (sequences != null) {
    ...
    } else if (sequences.source.value == "bedfile") {
      num_sequences = parseFloat(document.getElementById("sequences_bedfile_sequence_count").value);
  ...
  if ($("generate_off").checked) {
	...
	num_control_sequences = control_sequences.file_dbh.sequence_count;
      } else if (control_sequences.source.value == "bedfile") {
        num_control_sequences = parseFloat(document.getElementById("control_sequences_bedfile_sequence_count").value);
